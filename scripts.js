...
const observerOptions = {
    root: null, // Observa na janela de visualização (viewport)
...
const observerCallback = (entries, observer) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            entry.target.classList.add('visible');
            observer.unobserve(entry.target); // Para observar o elemento depois de ele ter sido visto
        }
    });
};
...
// Começa a observar os membros da equipe
